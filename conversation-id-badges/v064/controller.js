(() => {
  ...
  class HeaderController {
    ...
    refresh() {
      ...
      // Decide native-vs-missing from the native header structure itself,
      // BEFORE mounting our UI. This avoids contaminated sidebar/meta text from
      // making a complete native breadcrumb look like a missing title.
      const nativeComplete = this.adapter.hasNativeConversationBreadcrumb(mountInfo.headerRoot);

      const projectText = this.adapter.readableText(nextLane);
      const documentTitle = this.adapter.exactTitleFromDocument(projectText);
      const sidebarTitle = this.adapter.readSidebarTitle();
      const title = this.adapter.finalCleanTitle(documentTitle || sidebarTitle);
