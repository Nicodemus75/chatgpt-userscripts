  ...
  class HeaderView {
    ...
    installStyles() {
      ...
      s.textContent = `
        ...
        #${M.ROOT_ID}[data-mode="badge-only"]{flex:0 0 auto!important;width:48px!important;max-width:48px!important;margin-left:7px!important;overflow:visible!important;}
        #${M.ROOT_ID}[data-mode="hidden"]{display:none!important;}
        #${M.ROOT_ID} .cgpt-header-title-v069{display:block!important;flex:1 1 auto!important;min-width:0!important;width:auto!important;max-width:100%!important;overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important;text-align:left!important;font:inherit!important;line-height:1.3!important;padding:3px 6px!important;border-radius:6px!important;background:var(--cgpt-header-bg,var(--main-surface-primary,var(--background-primary,Canvas)))!important;color:var(--cgpt-header-title-color,var(--text-secondary,rgba(180,180,180,.95)))!important;opacity:1!important;box-sizing:border-box!important;}
        #${M.ROOT_ID}[data-context="regular"] .cgpt-header-title-v069{color:var(--cgpt-header-regular-title-color,var(--text-primary,rgba(245,245,245,.96)))!important;}
        #${M.ROOT_ID} .cgpt-header-badge-v069{display:inline-flex!important;align-items:center!important;justify-content:center!important;flex:0 0 40px!important;width:40px!important;min-width:40px!important;max-width:40px!important;height:18px!important;padding:0 4px!important;box-sizing:border-box!important;border:1px solid rgba(128,128,128,.38)!important;border-radius:5px!important;background:rgba(32,32,32,.88)!important;color:rgba(235,235,235,.88)!important;opacity:.72!important;font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Liberation Mono","Courier New",monospace!important;font-size:9px!important;font-weight:600!important;line-height:16px!important;cursor:copy!important;user-select:none!important;}
        #${M.ROOT_ID} .cgpt-header-badge-v069:hover{opacity:.94!important;background:rgba(55,55,55,.96)!important;border-color:rgba(160,160,160,.76)!important;}
    ...
    ensureRoot() {
      ...
      root.id = M.ROOT_ID;
      root.dataset.version = '0.6.9';
      const title = document.createElement('span');
      title.className = 'cgpt-header-title-v069';
      const badge = document.createElement('span');
      badge.className = 'cgpt-header-badge-v069';
    ...
    render({ mode, context, title, id, tag, maxWidth, headerBg, titleColor, regularTitleColor }) {
      ...
      if (mode === 'badge-only') {
        ...
        root.style.maxWidth = '48px';
        root.style.flexBasis = '48px';
        this.title.hidden = true;
        this.title.textContent = '';
      } else {
        const width = Math.max(0, Math.floor(Number(maxWidth) || 0));
        root.style.width = `${width}px`;
        root.style.maxWidth = `${width}px`;
        root.style.flexBasis = `${width}px`;
