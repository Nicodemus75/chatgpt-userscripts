(() => {
  ...
  class HeaderController {
    ...
    safeWidth(mountInfo) {
      ...
      const controlLeft = this.adapter.firstRightControlLeft(mountInfo.headerRoot, mountInfo.titleLane, rootRect.left);
      const rightLimit = Math.min(
        Number.isFinite(controlLeft) ? controlLeft - 18 : Infinity,
      ...
      const geometryWidth = Math.max(0, Math.floor(rightLimit - rootRect.left));
      return Math.max(0, Math.min(geometryWidth, Math.floor(headerRect.width * 0.52), 620));
    }
    ...
    refresh() {
      ...
      const rawTitle = sidebarTitle || this.adapter.titleFromDocument(fallbackProject);
      const title = this.adapter.cleanConversationTitle(rawTitle);
      if (!title) return;
