...
(() => {
  ...
  function positionBadges() {
    ...
    for (const [link, state] of tracked) {
      ...
      const visible = rect.width > 20 && rect.height > 10 && rect.bottom > 0 && rect.top < viewportHeight && rect.right > 0 && rect.left < viewportWidth;
      if (!visible) {
        badge.style.display = 'none';
        continue;
      }
      ...
      const left = Math.max(rect.left + 4, rect.right - badgeWidth - CONFIG.rightInsetPx);
